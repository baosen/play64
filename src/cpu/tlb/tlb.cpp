...
namespace {
	array<Entry, NENTRIES> tlb_entries = {0}; // 56 pages is random and 8 is hard-wired.
    ...
    void dbgprint() {
        for (int i = 0; i < tlb_entries.size(); ++i)
            cout << hex << setw(8) << i << ": " << tlb_entries[i].vpn2 << endl;
    }
}
...
namespace Tlb {
    ...
    physical_memory_address map(const memory_address virtual_memory_address) {
        ...
        cp0[BadVAddr] = virtual_memory_address;

        // Set Virtual Page Number.
        cp0[EntryHi] = virtual_memory_address & 0xffffe000;

        // TODO: wtf?
        cp0[Context] = 0xff80000f | ((virtual_memory_address >> 9) & 0x7ffff0);
